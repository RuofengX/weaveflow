# weaveflow — agent-oriented data engine（智能体的上下文外数据引擎）
...
├── lib.rs               # library facade: dsl/engine/error/operator/quickjs/store/tracker/vm/routine
...
│   │                     PIPELINE/TASK/SNAPSHOT/OBJECT/CACHE/ROUTINE/ROUTINE_EVENT tables + prune
│   │                     + v0 auto-migration + legacy trigger→routine 表迁移
│   ├── database.rs       redb table defs + (de)serialization helpers + SnapshotHeader view
│   │                     + EventKey ([name_len u32][name][seq u64] 复合 key)
│   └── object.rs         ObjectDigest (SHA256) + ObjectValue (all inline, no spill files, no ref_count)
├── routine/             # RoutineDef/RoutineRow/RoutineEventRecord 纯数据类型 + 校验 + cron 调度
│                        # 计算（lib 层，CLI 与 daemon 共用；运行时 worker 在 server/routine.rs）
...
│   │                     + submit_run 统一任务提交路径（HTTP /runs 与 routine worker 共用）
│   │                     + 三个终态收口点统一调 routine::on_task_terminal（收件箱 + webhook）
│   ├── routine.rs        Routine 运行时：RoutineManager（worker 注册表 + 全局事件 broadcast）、
│   │                     cron worker（misfire 策略）/ stream worker（微批缓冲 + max_in_flight
│   │                     信号量 + buffer_cap 背压）+ /routines HTTP API + WS 事件流
│   │                     + 事件持久化收件箱（append_routine_event）+ webhook 投递（deliver_webhook）
...
└── cli/                 # CLI client side (binary-only)
    ...
    ├── mcp.rs            MCP stdio server（rmcp，17 tools 薄客户端，与 CLI 共用 HTTP 层）
...
weaveflow mcp                      # MCP stdio server（rmcp，17 tools，面向 AI agent）
...
weaveflow routine apply -f <file.toml>   # TOML（CLI 本地解析）→ PUT JSON；daemon 不接触 TOML
weaveflow routine ls                     # alias: list；含 total_fired/next_fire 等运行时状态
weaveflow routine inspect <name>
weaveflow routine delete <name>
weaveflow routine push <name> -d '[...]' # stream 型；单对象自动包一层数组
weaveflow routine events <name> [--after N]  # 事件收件箱增量回查（seq 游标）
...
- Seven tables pre-created at `Database::open`; schema-versioned type names (`::v1`, Snapshot `::v2`). Opening a v0 database auto-migrates: backup to `<file>.v0.bak`, copy PIPELINE/TASK (stripping removed `snapshot_ttl`), drop SNAPSHOT/OBJECT/CACHE. 旧 `trigger` 表（2.0 之前）打开时自动迁移为 `routine` 表（RoutineRow 是超集，新增字段 serde default）后删除旧表。
...
- `TaskMeta.routine_source`: `Some("manual")` (HTTP /runs) or `Some("routine:<name>")`; old rows default `None`（serde alias 兼容旧字段名 `trigger_source`）。
...
- stream 并发闸：per-routine `max_in_flight` 信号量（默认 4），permit 由 1s 轮询 task 终态的后台任务释放；缓冲上限 `buffer_cap`（默认 10 万元素），到顶 push → 429。缓冲纯内存，重启丢弃未 flush 数据。
- 事件流：RoutineManager 全局 broadcast（容量 64），`/routines/:name/ws` 按名过滤；broadcast 是尽力通道，可靠回查走收件箱。
- PUT 已存在 routine = 热更新（停旧 worker 起新 worker，计数与近期 task 保留，stream 缓冲 flush 后重建）；DELETE 同时清空收件箱。
...
| GET | `/runs/:task_id` | Task status + progress（`?summary=1` = token 友好：无 inputs、Completed 内嵌输出替换为 true） |
| WS | `/runs/:task_id/ws` | Real-time progress push (TaskSnapshot JSON) |
| GET | `/runs/:task_id/snapshots` · `/:seq` | Snapshots（`?max_bytes=N` 截断超长输出为头部预览） |
...
| PUT | `/routines/:name` | Upsert routine（JSON body = RoutineDef；幂等） |
| GET | `/routines` · `/routines/:name` | Routine 列表（含运行时状态）/ 详情（含 buffered、近期 task_id） |
| DELETE | `/routines/:name` | 删除并停 worker（stream worker 先 flush 剩余缓冲再退出）+ 清空收件箱 |
| POST | `/routines/:name/push` | stream 入口：JSON 数组或单值（自动包一层）；缓冲满（buffer_cap，默认 10 万）→ 429 |
| GET | `/routines/:name/events?after=&limit=` | 事件收件箱增量回查（智能体值班回查通道；seq 游标） |
| WS | `/routines/:name/ws` | Routine 事件流（全局 broadcast 按名过滤） |
