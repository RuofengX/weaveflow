...
支持的 `operator`：`eq` `ne` `gt` `gte` `lt` `lte` `in` `contains`。其他值在 validator 校验阶段报错（`invalid_operator_config`），绕过校验运行时返回 Config 错误。`in` 要求 `value` 为数组、`contains` 要求 `value` 为字符串，否则运行时返回 Config 错误（不会静默返回空数组）。
...
注意：**非零退出码不会使步骤失败**——步骤照常 Completed，由下游通过 `exit_code` / `success` 字段判断（例如接一个 js 步骤断言）。这样管道可以主动消费预期的非零退出（如 `grep` 无匹配返回 1）。
...
| `mime_type` | RefValue | — | `"image/png"` | 图片 MIME 类型，必须是 `type/subtype` 形式（如 `image/png`）；字面量由 validator 校验（只拒绝不修改），引用形式（如 `{file.output.mimetype}`）在运行时校验 |
| `max_tokens` | u64 | — | `4096` | 最大输出 token |
| `temperature` | f64 | — | — | 采样温度 |
| `skip_vision_check` | bool | — | — | 跳过视觉模型检测 |
| `api_key` | RefValue | — | — | API 密钥，发送为 `Authorization: Bearer <key>`。**禁止明文**：validator 拒绝字面量，必须用引用（`{env.XXX}` / var / file 步骤输出） |
