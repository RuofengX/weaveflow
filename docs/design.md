...
weaveflow 是一个 **DAG 批处理引擎**，面向 AI Agent 和数据处理场景。

核心场景：

- **ETL 管道**：从 API 拉取数据 → 过滤/变换 → 写入目标
- **数据分批处理**：大批量记录切分后逐元素处理
- **LLM 链**：多步 prompt 串联 → 聚合结果
- **内联 JS**：QuickJS 沙箱执行 DSL 中内联的 JS 代码（`inputs.code`）

设计目标：

1. **纯 Rust 单二进制** — `weaveflow` CLI + daemon（Axum HTTP）
...
1. **内置算子** — 编译期注册，名字查找（`http`, `js`, `filter`, `sort`, `dedup`, `merge`, `base64`, `var`, `file`, `command`, `llm` 等 12 个）
2. **内联 JS** — DSL 中 `type: js` + `inputs.code`，Executor 创建 `JsOperator` 执行
...
| 错误处理 | thiserror + WeaveflowError 统一枚举 |
